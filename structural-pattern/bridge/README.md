...
**Bridge** là design pattern dạng structural giúp bạn tách một lớp khổng lồ hoặc một tập hợp lớp có quan hệ gần gũi với nhau thành hai hệ phân cấp lớp riêng biệt là - abstraction(trừu tượng) và implementation(triển khai) - có thể phát triển độc lập với nhau.

![intent](./assets/intent.png)

## 😟 Vấn đề

Giả sử bạn có một lớp hình học `Shape` với các lớp con: `Circle` và `Square`. Bạn muốn mở rộng hệ phân cấp lớp này cho kết hợp với màu sắc, kế hoạch của bạn là tạo lớp con `Red` và `Blue`. Tuy nhiên, vì bạn đã có hai lớp con rồi, nên bạn sẽ cần tạo bốn lớp kết hợp, vd như `BlueCircle` và `RedSquare`.

![problem](./assets/problem.png)

*Số lượng kết hợp tăng lên theo tiến trình hình học.*

Thêm một hình dạng mới hay màu sắc vào hệ phận cấp sẽ làm nó phát triển theo cấp số mũ. Ví dụ, thêm hình tam giác vào bạn sẽ cần thêm hai lớp con cho mỗi màu, sau đó nếu thêm một màu vàng vào sẽ lại cần thêm ba lớp con cho từng hình dạng. Càng thêm nhiều thì mọi thứ càng tệ đi.
...
Bạn có thể làm cho mọi thứ ổn định hơn bằng cách trích xuất code liên quan đến interface-platform thành các lớp riêng biệt. Tuy nhiên, sẽ có rất nhiều lớp trong đấy. Hệ phân cấp lớp sẽ phát triển theo cấp số mũ khi thêm một GUI hay hỗ trợ một API khác, vì sẽ yêu cầu tạo thêm nhiều và nhiều hơn các lớp nữa.

Vấn đề này sẽ được giải quyết bằng Bridge, khi ta chia các lớp thành hai hệ phân cấp lớp: 
...
Kết quả là, bạn có thể thay đổi lớp GUI mà không cần đụng đến lớp API liên quan. Hơn thế nữa khi thêm hỗ trợ cho một hệ điều hành khác chỉ yêu cầu tạo lớp con cho hệ phân cấp implementation.

## 🏢 Cấu trúc

![structure](./assets/structure.png)

1. **Abstraction** cung cấp logic điều khiển cấp cao. Nó dựa vao các đối tượng implementation để thực hiện các công việc ở cấp thấp hơn.
...
Lớp cơ sở của `Remote` khai báo trường tham chiếu liên kết với đối tượng `Device`. Tất cả điều khiển làm việc với thiết bị thông qua interface `Device` chung, nói giúp các cùng một điều khiển hỗ trợ nhiều loại thiết bị.
...
// hai hệ phân cấp lớp. Nó duy trì một liên kết đến đối 
// tượng của hệ phân cấp "implementation" và uỷ thác
...
// Bạn có thể mở rộng lớp từ hệ phân cấp abstraction
...
⚡ Ví dụ như khi bạn làm việc với server của các cơ sở dữ liệu khác nhau. Lớp có thể trở nên quá to và khó khăn để tìm hiểu các nó hoạt động, và khó hơn nữa để thay đổi nó. Các tay đổi với một biến thể của chức năng có thể yêu cầu thay đổi lên toàn bộ lớp, kết quả có thể tạo ra lỗi hoặc các tác dụng phụ nghiệm trọng.

Bridge giúp bạn tách khối lớp thành các hệ phân cấp lớp. Sau đó bạn có thể thay đổi lớp của một hệ phận cấp độc lập với các lớp khác. Cách tiếp cận này giúp code dễ bảo trì và tối thiểu nguy cơ code đang có bị ảnh hưởng.

**🐞 Sử dụng Bridge nếu bạn muốn mở rộng các lớp độc lập với nhau**

⚡ Bridge giúp bạn nên trích xuất một hệ thống phân cấp lớp riêng biệt cho từng phần. Lớp ban đầu ủy quyền công việc liên quan cho các đối tượng thuộc các phân cấp đó thay vì tự làm mọi thứ. 
